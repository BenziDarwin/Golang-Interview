$(document).ready(async function() {
    ...
    const response = await fetch(`http://localhost:6060/api/v1/patients/${patientId}`);
    if (!response.ok) {
                ...
                <table class="accordion-table">
                    ...
                    <tbody>
                        ${diagnoses.map((diagnosis, index) => {
                            const primaryBadge = index === 0 ? '<span class="primary-badge">Primary</span>' : '';
    ...
    function renderTreatmentsTable() {
        console.log('Rendering treatments table with', treatments.length, 'treatments');
                ...
                <table class="accordion-table">
                    ...
                    <tbody>
                        ${treatments.map((treatment, index) => {
                            const treatmentTypes = treatment.types ? treatment.types.split(',').map(t => t.trim()) : [];
                                        ...
                                        <td style="width: 30%;">
                                            <div class="cell-content">
                                                <div class="treatment-types-preview">
                                                    ${treatmentTypes.slice(0, 2).map(type => `<span class="treatment-type-badge">${type}</span>`).join('')}
                ...
                <table class="accordion-table">
                    ...
                    <tbody>
                        ${referrals.map((referral, index) => {
                            const statusClass = referral.status === 'Completed' ? 'status-completed' : 
                                               referral.status === 'Pending' ? 'status-pending' : 'status-inactive';
    ...
    // Modal events
    $(document).on('click', '#close-diagnosis-modal, #cancel-diagnosis-btn', function() {
        closeDiagnosisModal();
    });
